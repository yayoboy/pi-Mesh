...
**2026-07-12 sera — Screenshot KMS + status bar toggle (fatto):** (1) screenshot funzionanti sul kiosk KMS: ffmpeg kmsgrab cattura lo scanout DRM (VC4 T-tiled, modifier 0x0700000000000001) e scripts/vc4_detile.py rimette i tile in ordine raster (ordine subtile validato empiricamente: even TL,BL,BR,TR / odd BR,TR,TL,BL, boustrofedone; roundtrip-tested). Richiede ffmpeg+python3-numpy+python3-pil sul Pi (installati). Neo noto: angolo basso-destra ~256×24px nero (dump troncato, H non multiplo di 32). (2) Tap sul logo in alto a sinistra = toggle status bar 32px↔52px (icone 24px, target 44px), preferenza ui.statusbar_large persistita lato server. Verificato entrambi end-to-end.

**2026-09-18 — Orange Pi 4 Pro, kiosk o cyberdeck (fatto, da verificare sulla scheda):** target Allwinner A733 con immagine ufficiale Debian/Ubuntu, monitor HDMI, radio USB. Audit del codice: quattro rotture reali fuori dal Raspberry. (1) pimesh.service, kiosk-hdmi.service e start-kiosk-hdmi.sh avevano utente `pimesh` e `/home/pimesh/pi-Mesh` hardcoded → setup.sh e setup-display-hdmi.sh li riscrivono con sed per l'utente che lancia sudo e il percorso reale del repo. (2) setup-display-hdmi.sh moriva (set -e) copiando un config.txt inesistente → lo step vc4/CMA/tft35a gira solo se il file esiste. (3) Su Debian/Ubuntu non esiste il gruppo gpio → setup.sh lo crea con regola udev per /dev/gpiochip*. (4) vc4_detile.py avrebbe rimescolato i frame lineari di sun4i-drm → detile solo se /sys/module/vc4 esiste (test in tests/test_screenshot.py). Launcher desktop: fallback xdg-open senza Chromium. README §7. **Da verificare sull'Orange Pi:** `ls /dev/dri/card*` (cog -P drm vuole KMS+GBM sul kernel vendor, non confermabile da qui; ripiego = profilo desktop), numeri di linea GPIO con `gpioinfo`, `cog` da universe su Ubuntu. Non toccati: setup-rtc.sh, optimize-pi.sh, rotazione via config.txt (no-op fuori dal Pi).
